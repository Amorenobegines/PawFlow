...
<html>
    ...
    <body>
        ...
        <form action="{{ $isEdit ? route('appointments.update', $appointment->id) : route('appointments.store') }}" method="POST">
            @csrf
            @if ($isEdit)
                @method('PUT')
            @endif
            ...
            <input type="date" name="date" id="date" value="{{ old('date', $isEdit ? $appointment->date : '') }}" required><br>
            ...
            <select name="time" id="time" required>
                ...
                @for ($hour = 9; $hour < 21; $hour++)
                    @for ($minute = 0; $minute < 60; $minute += 30)
                        ...
                        @endphp
                        <option value="{{ $value }}" {{ old('time', $isEdit ? $appointment->time : '') == $value ? 'selected' : '' }}>
                            {{ $value }}
                        </option>
                    @endfor
                @endfor
            ...
            <select name="status" id="status" required>
                ...
                <option value="pending" {{ old('status', isset($appointment) ? $appointment->status : '') === 'pending' ? 'selected' : '' }}>Pending</option>
                <option value="completed" {{ old('status', isset($appointment) ? $appointment->status : '') === 'completed' ? 'selected' : '' }}>Completed</option>
                <option value="canceled" {{ old('status', isset($appointment) ? $appointment->status : '') === 'canceled' ? 'selected' : '' }}>Canceled</option>
            </select><br>
            ...
            <select name="client_id" id="client_id" required>
                ...
                @foreach($clients as $client)
                    <option value="{{ $client->id }}" {{ old('client_id', isset($appointment) ? $appointment->client_id : '') == $client->id ? 'selected' : '' }}>
            ...
            <select name="pet_id" id="pet_id" required>
                ...
                @foreach($pets as $pet)
                    <option value="{{ $pet->id }}" {{ old('pet_id', isset($appointment) ? $appointment->pet_id : '') == $pet->id ? 'selected' : '' }}>
                        {{ $pet->name }}
                    </option>
                @endforeach
            </select><br>
                ...
                <input type="checkbox" name="service_ids[]" id="service_ids" value="{{ $service->id }}" {{ in_array($service->id, old('service_ids', isset($appointment) ? $appointment->services->pluck('id')->toArray() : [])) ? 'checked' : '' }}>
            ...
            <select name="user_id" id="user_id" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ old('user_id', isset($appointment) ? $appointment->user_id : '') == $user->id ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select><br>
                ...
                @foreach($appointments as $appointmentItem)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('appointments.destroy', $appointmentItem->id) }}"
                                    method="POST"
